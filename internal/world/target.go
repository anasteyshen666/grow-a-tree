...
// Damage destroys a gnawed root, so the network beyond it falls off. Core
// damage (and the resulting game over) is handled in a later stage.
func (g *Grid) Damage(col, row int) {
	if g.Kind(col, row) == Root {
